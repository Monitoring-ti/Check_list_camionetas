...
export default function ChecklistWizard() {
    ...
    <div className="step-body">
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .gestion-vial-toggle-card {
          ...
          padding: 1.15rem; background: #fff; border: 1px solid var(--border); border-radius: 12px;
          border-left: 4px solid var(--orange);
        }
        .toggle-info { display: flex; gap: 1rem; align-items: center; flex: 1; min-width: 0; }
        .toggle-info p { margin: 0; font-size: 0.85rem; color: var(--text-muted); }
        ...
        .toggle-switch label {
          ...
          background: #cbd5e1; transition: .3s; border-radius: 4px;
        }
        .toggle-switch label:before {
          content: ""; position: absolute; height: 18px; width: 18px; left: 4px; bottom: 4px;
          background: white; transition: .3s; border-radius: 2px;
        }
  ...
  return (
    <div className="app-shell">
      <AppHeader
        variant="bar"
        subtitle="Inspección de unidad"
        badge={hasBadBlocking ? (
          <div className="header-alert-badge"><AlertTriangle size={14} /> No Apto</div>
        ) : undefined}
        meta={
          <>
            <span><strong>Patente:</strong> {session.vehiculo.patente}</span>
            <span><strong>Operador:</strong> {session.trabajador.nombre}</span>
          </>
        }
      />
